...
parser$add_argument("--relatedness_file", default="~/01_repos/GWAS_pipeline/data/datasets/ukb11350_rel_s488226.dat")
...
parser$add_argument("-od", "--output_dir", default="~/01_repos/CardiacMotionGWAS/results")
parser$add_argument("--tmpdir", default="tmp/")
parser$add_argument("--seed", default=42, nargs="+")
parser$add_argument("--replication_fraction", default=0.1, nargs="+")
parser$add_argument("--gzip", default=FALSE, action="store_true", help="Whether to gzip the output files.")
...
parser$add_argument("--debugmode", default=FALSE, action="store_true", help="Whether to output debug messages.")
...
fracs_replication <- as.numeric(args$replication_fraction)

SAMPLES_COLS <- c("id_1", "id_2", "missing")
samples_df <- readr::read_table(samples_file, skip = 2, col_names = SAMPLES_COLS, show_col_type=FALSE)
...
rel_df <- read_delim(rel_file, show_col_type=FALSE)
...
dir.create(g("{args$tmpdir}/GreedyRelated/"), showWarnings=FALSE)
...
for (SEED in SEEDS) {
  ...
  dir.create(g("{args$output_dir}/seed_{SEED}/"), showWarnings=FALSE)
  ...
  for (frac_replication in fracs_replication) {
    ...
    set.seed(SEED)
    logging::loginfo("Using {100*frac_replication}% samples for replication" %>% g)
    
    ids_discovery <- samples_df %>% sample_frac(size = 1-frac_replication) %>% .$ID
    logging::logdebug("Number of discovery IDs: {length(ids_discovery)}")
   
    logging::logdebug("Before filtering: {nrow(rel_df)}" %>% g)
    ...
    logging::logdebug("After filtering: {nrow(subset_rel1_df)}" %>% g)
    intermediate_rel1 <- "{args$tmpdir}/GreedyRelated/greedy_relatedness_discoveryIDs_frac{100*(1-frac_replication)}.dat" %>% g

    logging::logdebug("Subjects: {intermediate_rel1}" %>% g)
    write_delim(subset_rel1_df, intermediate_rel1, delim="\t")
    
    exclude_subjects_file <- "{args$tmpdir}/GreedyRelated/exclude_subjects_discovery_{1-frac_replication}.txt" %>% g
    
    logging::loginfo("Producing a list of excluded subjects from discovery set: {exclude_subjects_file}" %>% g)
    greedy_related_cmd1 <- "{GreedyExec} -r {intermediate_rel1} -seed 1 --id1 ID1 --id2 ID2 -f Kinship -o {exclude_subjects_file}" %>% g
    logging::loginfo("Running GreedyRelated command: {greedy_related_cmd1}" %>% g)
    greedy_output_1 <- system(greedy_related_cmd1, intern=TRUE, ignore.stderr=!args$debugmode)
    # The file does not have a header (hence col_names=FALSE)
    exclude_subjects <- read_delim(exclude_subjects_file, col_names=FALSE, show_col_type=FALSE)
    exclude_subjects <- exclude_subjects$X1
    logging::loginfo("Excluding subjects from discovery dataset: {length(exclude_subjects)}" %>% g)
    ...
    intermediate_rel2 <- "{args$tmpdir}/GreedyRelated/greedy_relatedness_replicationIDs_frac{100*(1-frac_replication)}.dat" %>% g
    
    logging::logdebug("Subjects: {intermediate_rel2}" %>% g)
    write_delim(subset_rel2_df, intermediate_rel2, delim="\t")
    
    exclude_subjects_file2 <- "{args$tmpdir}/GreedyRelated/exclude_subjects_replication_{1-frac_replication}.txt" %>% g
    
    logging::loginfo("Producing a list of excluded subjects from replication set")
    greedy_related_cmd2 <- "{GreedyExec} -r {intermediate_rel2} -seed 1 --id1 ID1 --id2 ID2 -f Kinship -o {exclude_subjects_file2}" %>% g
    greedy_output_2 <- system(greedy_related_cmd2, intern=TRUE, ignore.stderr=TRUE)
    
    exclude_subjects <- read_delim(exclude_subjects_file2, col_names=FALSE, show_col_type=FALSE)
    exclude_subjects <- exclude_subjects$X1
    logging::loginfo("Excluding subjects from replication dataset: {length(exclude_subjects)}" %>% g)
  ...
  SAMPLES_FILES <- args$bgen_sample_file
  # samples
  names(SAMPLES_FILES) <- nrow(readr::read_table(args$bgen_sample_file, skip = 2, col_names = SAMPLES_COLS, show_col_type=FALSE))

  for (sample_file_n in names(SAMPLES_FILES)) {
    
    sample_file <- SAMPLES_FILES[sample_file_n]
    samples_df <- readr::read_table(sample_file, skip = 2, col_names = SAMPLES_COLS, show_col_type=FALSE)
    samples_df <- as.data.frame(samples_df)
    names(samples_df)[1] <- "ID"
    ...
    for (frac_replication in fracs_replication) {
      ...
      logging::loginfo("{length(ids_replication)} + {length(ids_discovery)} subjects for replication and discovery (replication fraction {100*frac_replication}%)" %>% g)
      ...
      pheno_df <- pheno_df %>% rename("ID"="pheno_df$ID")
      logging::loginfo("Creating final phenotype file for discovery: {ofile_discovery}" %>% g)
      ...
      pheno_df <- pheno_df %>% rename("ID"="pheno_df$ID")
      logging::loginfo("Creating final phenotype file for replication: {ofile_replication}" %>% g)
